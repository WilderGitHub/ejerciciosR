...
# Periodos de TOTALES
primer <- as.Date("31-12-08",format="%d-%m-%y")
segundo <- as.Date("31-12-15",format="%d-%m-%y")

periodos<-combenado %>% mutate(Periodo =
                     case_when(Fecha <= primer ~ "2005-2008", 
                               Fecha <= segundo ~ "2009-2015",
                               Fecha >segundo ~ "2016-2020"))
                     
ggplot(periodos, aes(x=IEDTotal, y=PIBTotal, color=Periodo)) +
...
periodos<-periodosSectores %>% mutate(Periodo =
                                 case_when(Fecha <= primer ~ "2005-2008", 
                                           Fecha <= segundo ~ "2009-2015",
                                           Fecha >segundo ~ "2016-2020"))

ggplot(periodosSectores, aes(x=IED, y=PIBTotal, color=Periodo)) +
